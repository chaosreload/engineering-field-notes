...
## 同类工具对比：claude-hud vs ccstatusline

Claude Code statusline 生态中，[ccstatusline](https://github.com/sirmalloc/ccstatusline) 是另一个活跃的开源项目。两者解决同一个问题——在终端底部显示 Claude Code 运行状态——但设计哲学截然不同。

### 对比总览

| 维度 | claude-hud | ccstatusline |
|------|-----------|--------------|
| **定位** | 轻量信息面板，够用就好 | 高度可定制的美化工具 |
| **安装方式** | Claude Code Plugin（`/plugin install`） | npm 包（`npx ccstatusline`） |
| **配置方式** | Markdown 斜杠命令 + JSON 配置 | 交互式 TUI（React/Ink），可视化编辑 |
| **UI 风格** | 固定布局，ANSI 彩色文本 | Powerline 箭头、主题、Nerd Font、Truecolor |
| **Widget 丰富度** | 核心指标（模型、context、工具、agent、todo） | 20+ widget：Git 全家桶、Session Cost、Block Timer、CWD、Memory、Vim Mode、Token Speed、Skills、Thinking Effort… |
| **用量追踪** | context 百分比 + rate limit bar | Session/Weekly Usage、Block Reset Timer，可调 Anthropic API |
| **运行时依赖** | **零依赖**（仅 devDependencies） | React/Ink 等 npm 依赖 |
| **测试** | 266 个测试，纯单元测试（依赖注入） | 有测试，侧重功能迭代 |
| **跨平台** | macOS / Linux | Windows / macOS / Linux / WSL 全平台 |
| **Bedrock 支持** | ✅ 自动识别模型名，隐藏不适用的订阅信息 | 未特别适配 |
| **迭代节奏** | 稳定版本 | 非常活跃（v2.0 → v2.2，密集更新） |

### 选择建议

- **选 ccstatusline**：你喜欢折腾终端美化、需要丰富的 widget 组合（Git diff 统计、Session Cost、Token Speed 等）、或在 Windows 上使用 Claude Code。
- **选 claude-hud**：你追求极简和零开销、想学习 Claude Code Plugin 的标准实现方式、或大量使用 AWS Bedrock。
- **两者共同点**：都利用 Claude Code 的 `statusLine` API（`settings.json` 中配置），都是实时刷新，都能显示模型名和 context 用量。

### 工程启发

即使不使用 claude-hud，它的两个设计模式值得借鉴：

1. **Markdown 即安装脚本**：`commands/setup.md` 不是 shell 脚本，而是给 Claude 读的操作指南。Claude 阅读后自动执行每一步——这是 "AI-native 工具链" 的思路。
2. **依赖注入实现全纯测试**：`MainDeps` 接口让所有 I/O 可 mock，266 个测试无需真实 Claude Code 环境。对任何 CLI 工具的测试架构都有参考价值。
